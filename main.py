...
def fetch_naver_news(keywords_map, display=10):
    ...
    naver_items = []
    for category_name, kw_list in keywords_map.items():
        for kw in kw_list:
            try:
...
def fetch_google_news(categories):
    prompt = f"""
다음 분야별 최신 주요 구글 뉴스 기사를 검색하세요:
분야: {', '.join(categories)}

각 분야별로 2개씩 총 주요 뉴스 8개를 선정하고, 오직 아래의 JSON 배열 형식으로만 답변해주세요.
...
[
  {{
    ...
    "link": "기사 URL",
    "press_name": "언론사명",
    "category": "정치/경제/사회/생활/문화/IT/과학 중 1개",
    "pub_time": "YYYY-MM-DD"
  }}
]
"""
